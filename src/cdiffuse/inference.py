    ...
    if not model_dir in models:
        if (model_dir / "weights.pt").is_file():
            checkpoint = torch.load(
                model_dir / "weights.pt", "cuda" if args.use_gpu else "cpu"
            )
        else:
            checkpoint = torch.load(model_dir, "cuda" if args.use_gpu else "cpu")
...
def main(args):
    ...
    for spec in specnames:
        ...
        fname = spec.name.rstrip(".spec.npy")
        print(fname)
        noisy_fname = args.wav_path / fname
        print(noisy_fname)
        ...
        output_name = args.output / spec.relative_to(args.spectrogram_path).rstrip(
            ".spec.npy"
        )
        print(output_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("wav_path", type=Path, help="input noisy wav directory")
    parser.add_argument(
        "--output", "-o", type=Path, default="output/", help="output path name"
    )
    parser.add_argument(
