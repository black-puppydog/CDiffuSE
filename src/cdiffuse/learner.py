...
class DiffuSELearner:
    ...
    def load_state_dict(self, state_dict, pretrain=False):
        if pretrain:
            ...
            for i in range(30):
                model_state_dict.pop(
                    "residual_layers.{}.conditioner_projection.weight".format(i), None
                )
        if hasattr(self.model, "module") and isinstance(self.model.module, nn.Module):
            if pretrain:
    ...
    def train_step(self, features):
        ...
        with self.autocast:
            ...
            noisy_audio = (
                (1 - m) * noise_scale_sqrt * audio
                + m * noise_scale_sqrt * noisy
                + (1.0 - (1 + m**2) * noise_scale) ** 0.5 * noise
            )
            combine_noise = (
                m * noise_scale_sqrt * (noisy - audio)
                + (1.0 - (1 + m**2) * noise_scale) ** 0.5 * noise
            ) / (1 - noise_scale) ** 0.5
...
def train_distributed(replica_id, replica_count, port, args, params):
    ...
    model = DistributedDataParallel(
        model, device_ids=[replica_id], find_unused_parameters=True
    )
    _train_impl(
        replica_id,
        model,
