...
def spec_transform(filename, indir, outdir):
    spec, _, _ = make_spectrum(
        filename, FRAMELENGTH=params.n_fft, SHIFT=params.hop_samples
    )
    ...
    if args.se:
        with ProcessPoolExecutor(max_workers=10) as executor:
            list(
                tqdm(
                    executor.map(
                        spec_transform, filenames, repeat(args.dir), repeat(args.outdir)
                    ),
                    desc="Preprocessing",
                    total=len(filenames),
                )
        ...
        with ProcessPoolExecutor(max_workers=10) as executor:
            list(
                tqdm(
                    executor.map(
                        transform, filenames, repeat(args.dir), repeat(args.outdir)
                    ),
                    desc="Preprocessing",
                    total=len(filenames),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "outdir", help="output directory containing .npy files for training"
    )
